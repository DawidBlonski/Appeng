...
class Test_vievs_status(TestCase):
    ...
    def test_login_user(self):
        for url in self.login_required_url:
            response = self.client.post(reverse(f'accounts:{url}'),self.credentials,follow=False)
...
class TestAccounts(StaticLiveServerTestCase):
    def setUp(self):
        ...
        self.register_url = ('%s%s' % (self.live_server_url, reverse_lazy("accounts:register")))
...
'''
    def test_account(self):
        self.client.login(username='test', password='TeSt412')
        force_login(self.user,self.browser,self.live_server_url)
        self.browser.get('%s%s' % (self.live_server_url, reverse_lazy("appeng:home")))
        self.browser.find_element_by_id("1000").click()
        self.assertIn(self.live_server_url, self.browser.current_url)'''
