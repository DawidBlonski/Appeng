...
from backend.courses.models import Courses
import random

user = get_user_model()


class RandomAnsers(models.Manager):
    def random(self, user, course):
        answer = self.values('word').filter(
            user=user, course=course, answer=False
        )
...
class Answers(models.Model):
    ...
    objects = RandomAnsers()

    @classmethod
    def create_answers(cls, course, user):
        word_in_course = course.__class__.objects.values_list(
            'word', flat=True
        )
